...
class ClassificationTrainer:
    ...
    def train(self, classifier, trainloader, optimizer, num_epochs,
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            for phase in loaders:
                ...
                for batch_idx, data in enumerate(loaders[phase], 0):
                    pointcloud, label, index = data
                    pointcloud = pointcloud.permute(0, 2, 1)
                    # radius = radius.unsqueeze(1)
                    pointcloud, label = pointcloud.to(device), label.to(device)
        ...
        results = {
            ...
            "labels_dict": trainloader.dataset.classes_dict,
...
if __name__ == '__main__':
    ...
    from datasets.dstorm_datasets import NoasDstormDatasetSimpleXY
    ...
    batch_size = 16
    workers = 4
    path = '/home/oronlevy/ext/data/partitioned/Noas_Bassoon_Mem_and_Sub/Train/'

    dataset = NoasDstormDatasetSimpleXY(
        root='/home/oronlevy/ext/data/partitioned/Noas_Bassoon_Mem_and_Sub/Train/',
        radius=500,
        max_nsamples=400,
        min_npoints=128,
        coloc_distance=50,
    ...
    validloader = DataLoader(dataset, batch_size=batch_size, shuffle=False, num_workers=0,
                             sampler=valid_sampler, pin_memory=False)

    print(len(dataset))
    print(len(train_sampler), len(valid_sampler))
    print(len(trainloader), len(validloader))

    classifier = PointNetClassifier(features_len=1024, num_classes=2).float()
